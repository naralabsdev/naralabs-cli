...
export const DEFAULT_SCHEMA_FILE = "naralabs.schema.yaml";
export const LEGACY_SCHEMA_FILE = "naralabs.schema.json";

export function loadConfig(): CliConfig {
  if (!existsSync(CONFIG_PATH)) {
    return {
      apiUrl: process.env.NARALABS_API_URL ?? "http://localhost:8080",
  ...
  return {
    apiUrl: process.env.NARALABS_API_URL ?? file.apiUrl ?? "http://localhost:8080",
...
export function resolveSchemaPath(file?: string): string {
  ...
  const yamlPath = resolve(process.cwd(), DEFAULT_SCHEMA_FILE);
  if (existsSync(yamlPath)) return yamlPath;

  const jsonPath = resolve(process.cwd(), LEGACY_SCHEMA_FILE);
...
export function requireSchemaPath(file?: string): string {
  ...
  if (!existsSync(p)) {
    throw new Error(
      `No ${DEFAULT_SCHEMA_FILE} found. Run \`naralabs init <folder>\` or pass a file path.`,
    );
  }
  return p;
}
